...
public enum NoteSign {
    ...
    private static List<NoteSign> getVbNotesByMidi() {
        return Arrays.stream(values())
                .sorted(Comparator.comparing(NoteSign::getMidi))
                .collect(Collectors.toList());
    }

    public NoteSign someHigher(int diff) {
        List<NoteSign> collect = getVbNotesByMidi();
        int i = collect.indexOf(this);

        if (i + diff >= collect.size() - 1) {
            return collect.get(collect.size() - 1);
        }
        if (i + diff <= 0) {
            return collect.get(0);
        }
        return collect.get(i + diff);
    }
